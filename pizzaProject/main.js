...
const filterPriceCaloricity = () => {
    ...
    filterBtn[1].onclick = function () {
        let result = pizzaList.filter((pizza) => {
            ...
            if (valueFrom.value == '' && valueTo.value != '' && valueTo <= pizza.price) {
                return pizza
            }
            if (valueFrom.value != '' && valueTo.value <= pizza.price) {
                return pizza
            }
    ...
    filterBtn[0].onclick = function () {
        let result = pizzaList.filter((pizza) => {
            (pizza.caloricity);
            if (pizza.caloricity >= valueFrom.value && pizza.caloricity > valueFrom.value && pizza.caloricity < valueTo.value) {
                return (pizza);
            }
            if (valueFrom.value == '' && valueTo.value == '') {
                return (pizza);
            }
            if (valueFrom.value == '' && valueTo.value != '' && valueTo <= pizza.caloricity) {
                return pizza
            }
